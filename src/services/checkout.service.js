...
class CheckoutService {
  ...
  static async checkoutReview({ cartId, userId, shop_order_ids = [] }) {
    ...
    if (!foundShop) throw new BadRequestError(`Cart doesn't exist!`);
    ...
    for (let i = 0; i < shop_order_ids.length; i++) {
      ...
      const checkoutOrderServer = await checkProductByServer(item_products);
      console.log(
        "CheckoutService ~ checkoutReview ~ checkoutOrderServer:",
        checkoutOrderServer
      );
      ...
      const itemCheckout = {
        shopId,
        shop_discounts,
        priceRaw: checkoutPrice, // before add discount
        priceApplyDiscount: checkoutPrice, // after add discount
  ...
  static async orderByUser({
    ...
    // nếu tạo order thành công cần remove cart
